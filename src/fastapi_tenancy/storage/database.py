...
class SQLAlchemyTenantStore(TenantStore[Tenant]):
    ...
    async def _update_metadata_generic(
        ...
        """
        async with session.begin():
            # Set SERIALIZABLE isolation so that the DB aborts one of two
            # concurrent callers that both read-then-write the same metadata
            # row, preventing the lost-update race condition.
            # execution_options must be applied before any DML is issued.
            await session.connection(execution_options={"isolation_level": "SERIALIZABLE"})
